...
def add_case_json(): # 関数名を add_case_json などに変更すると、既存の add_case.html との混同を防ぎやすい
    try:
        ...
        return jsonify({'success': True, 'message': 'テストケースが正常に追加されました。', 'id': new_case.id})
    except Exception as e:
        db.session.rollback() # エラー時はロールバック
        logger.exception(f"[ERROR] Failed to add test case: {e}")
        return jsonify({'success': False, 'message': f'テストケースの追加中にエラーが発生しました: {str(e)}'}), 500
...
def edit_case_json(case_id): # 関数名を edit_case_json などに変更
    ...
    try:
        ...
        return jsonify({'success': True, 'message': 'テストケースが正常に更新されました。'})
    except Exception as e:
        db.session.rollback() # エラー時はロールバック
        logger.exception(f"[ERROR] Failed to update test case {case_id}: {e}")
        return jsonify({'success': False, 'message': f'テストケースの更新中にエラーが発生しました: {str(e)}'}), 500
...
def delete_multiple_testcases():
    try:
        ...
        return jsonify({'message': f'{deleted_count}件のテストケースを削除しました。', 'success': True}) # successキーを追加

    except Exception as e:
        db.session.rollback() # エラー時はロールバック
        logger.exception(f"[ERROR] delete_multiple_testcases: {e}")
        return jsonify({'message': f'⚠ 削除中にエラーが発生しました: {str(e)}', 'success': False}), 500
...
def start_codegen():
    try:
        ...
        return jsonify({"status": "started", "message": "Playwright codegenが起動しました。"})
    except Exception as e:
        logger.exception(f"[ERROR] Failed to start codegen: {e}")
        return jsonify({"status": "error", "message": f"コード生成ツール起動中にエラー: {str(e)}", 'success': False}), 500
...
def run_testcase():
    ...
    if not case_id:
        ...
        return jsonify({'message': 'IDが送信されていません', 'success': False}), 400

    testcase = TestCase.query.get(case_id)
    if not testcase or not testcase.code:
        logger.debug(f"Test case {case_id} not found or no code.")
        return jsonify({'message': 'テストコードが見つかりません', 'success': False}), 404
    ...
    try:
        ...
        command = f"pytest {os.path.normpath(temp_path)} --html={os.path.normpath(report_path)} --self-contained-html"
        ...
        logger.debug(f"Subprocess.run finished. Return code: {result.returncode}")
        ...
        if result.returncode == 0:
            ...
            return jsonify({
                'success': True,
                'message': '✅ テスト成功',
                'report_url': url_for('static', filename=os.path.join('reports', report_file_name)) # staticフォルダからの相対パス
            })
        else:
            logger.warning(f"Test failed for case_id {case_id}. Return code: {result.returncode}")
        ...
        return jsonify({'success': False, 'message': '⏱ 実行がタイムアウトしました。テストプロセスが終了したか確認してください。'})
    except Exception as e:
