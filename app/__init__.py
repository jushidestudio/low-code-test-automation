...
def create_app():
    ...
    # os.path.abspath(os.path.dirname(__file__)) は
    # 現在のファイル（__init__.py）のディレクトリパス（つまり app ディレクトリのパス）を取得
    # os.path.join でそのパスに 'static' を結合し、
    # app/static を静的ファイルのルートとして指定
    app = Flask(
                __name__, static_folder=os.path.join
